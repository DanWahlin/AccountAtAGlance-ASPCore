...
var tileFormatter = new function () {
    var previousPoint = null,
        ...
        formatQuoteData = function (data) {
            ...
            //val(
            //parseFloat(TESTE.toString().match(/^\d+(?:\.\d{0,2})?/)));
            quoteTileDiv.find('.Currency').formatCurrency({ symbol: '' });
        },

        formatMarketData = function(marketTileDiv) {
            var tileId = marketTileDiv.attr('id'),
                //Camel-case serializer will change SP500 to sP500 so account for this
                dataPrefix = (tileId.indexOf('500') > -1) ? 'sP500' : tileId.toLowerCase();
            ...
            if (canvasDiv.length > 0) {
                ...
                renderCanvas(canvasDiv, width, height, marketTileDiv.data().chartColor, data[dataPrefix], data[dataPrefix + 'DataPoints']);
            }
